...
- **История проекта** — новый RPC `get_project_activity(p_project_id uuid, p_limit int default 100)`
  ...
  1. **Гейт доступа**: **точное зеркало `projects_select`** (20260611_0002) —
     `owner OR is_admin() OR (visibility='team' AND is_project_member) OR (visibility='marketplace' AND is_approved())`.
     НЕ через `can_access_project_comments` — оно для `team` пускает любого approved (шире видимости проекта →
     утечка истории team-проекта посторонним). Нет доступа → пустой результат.
...
- **Гейт доступа в get_project_activity** реализован зеркалом `projects_select` (20260611_0002),
  НЕ `can_access_project_comments` (последнее для `team` пускает любого approved → утечка истории).

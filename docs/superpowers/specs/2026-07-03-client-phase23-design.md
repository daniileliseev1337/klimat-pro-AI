...
Следующий шаг: writing-plans → один план на обе фазы (Фаза 2 → Фаза 3), реализация в этом заходе.
...
4. **Scope — обе фазы в этом заходе** (владелец 03.07: «давай фазу 3 тут же»). Один implementation-план на
   обе: задачи Фазы 2 (переписка+файлы) идут первыми, Фаза 3 (уведомления) — надстройка после них (зависит
   от `post_client_message` / `set_file_client_visible` как точек вызова).

...
**Обновлено: 2026-07-29** (worktree `codex/personalized-ui`: реализованы account appearance, доводка банка v2 и UX; `npm test` = 157/157, `npm run build` = PASS. ✅ Миграция применена к живой БД, RLS policy структурно проверена. Merge/deploy/push ещё впереди.)

## В работе — персонализируемый UI (локально, не в main)
...
- Закрыты: «Помощь» в профиле и ложная подпись `Supabase (Frankfurt)`. Реальная авторизованная visual QA ожидает локальное безопасное окружение/сессию; production не трогался.

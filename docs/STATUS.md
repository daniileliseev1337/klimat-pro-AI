...
**Обновлено: 2026-07-04** (сессия: follow-up Фазы 3 `client_stage_changed` через форму проекта — в проде; умный импорт выписки Яндекс Банка v2 — spec+план+**реализация** (15 коммитов, 147 тестов, subagent-driven, финальный opus-review) в main `cbc0687`, **НЕ в проде** — ждёт прогона на реальной выписке + deploy/миграции по «го»; real-time push отложен)

## Прод
- Живой: https://193-124-130-236.sslip.io — HTTP 200 снаружи (проверено exa) и на nginx :8080.
- Прод-код: **a0c8542** (follow-up Фазы 3 `client_stage_changed` через форму проекта; asset
  `index-CG_qyP3W.js`, задеплоен 04.07, сверено dist=nginx:8080=снаружи, HTTP 200).
  GitHub main = **cbc0687** (⊃ банк v2, НЕ задеплоен — прод остаётся на `CG_qyP3W` до deploy по «го»).

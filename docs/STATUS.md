...
**Обновлено: 2026-07-29** (`codex/appearance-ui-fix`: исправлена visual-регрессия account appearance; `npm test` = 161/161, production build = PASS. Исправление ещё НЕ перенесено в `main` и НЕ задеплоено. Текущий production остаётся на `2e3d2d1` / asset `index-FdBhWjDr.js`.)

## В работе — исправление персонализируемого UI
...
- Visual QA на локальном стенде: desktop PASS; mobile 390 px PASS без горизонтального overflow; `Блик` подтверждён вычисленным `animation-name: kp-ui-shimmer`, `6s` и видимым gradient-layer. Авторизованные страницы проверены структурно и тестами; повторная visual QA на production нужна после отдельного разрешения на release.

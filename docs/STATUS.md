...
**Обновлено: 2026-07-04** (сессия: follow-up Фазы 3 `client_stage_changed` — в проде; умный импорт выписки Яндекс Банка v2 — spec+план+**реализация** (15 коммитов, 147 тестов, subagent-driven, opus-review) merge в main + **задеплоен в стек** (nginx `CRXk9JKF`) + миграция `merchant_rules` применена к живой БД по «го». ⚠️ Прод СНАРУЖИ 502 — frp-туннель порван (VPN Direct-правило, действие владельца), внутренний стек здоров. Банк НЕ прогнан на реальной выписке. real-time push отложен)

## Прод
- ⚠️ Снаружи https://193-124-130-236.sslip.io — **HTTP 502** (frp-туннель ПК→VPS порван: `journalctl -u frpc` =
  `connection write timeout` / `i/o timeout 193.124.130.236:7000` серией). Внутренний стек ЗДОРОВ: nginx :8080 = 200.
  Причина — VPN Happ без Direct-правила `193.124.130.236/32` (грабли CLAUDE.md). **Действие владельца:** вернуть
  Direct в VPN-профиль → frpc реконнектится сам (рестарт не нужен). Это НЕ проблема деплоя.
- Прод-код: **26e0b86** (банк v2 — умный импорт выписки; asset `index-CRXk9JKF.js`, задеплоен в nginx :8080 04.07,
  dist=nginx совпали; снаружи не сверен из-за 502). GitHub main = 26e0b86. Предыдущий прод — `CG_qyP3W` (follow-up Фазы 3).
  ⚠️ Банк НЕ прогнан на реальной выписке (эвристики проверены синтетикой) — проверить первый импорт когда туннель вернётся.

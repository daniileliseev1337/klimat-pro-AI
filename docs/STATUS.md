...
**Обновлено: 2026-07-04** (сессия: follow-up Фазы 3 `client_stage_changed` — в проде; умный импорт выписки Яндекс Банка v2 — spec+план+**реализация** (15 коммитов, 147 тестов, subagent-driven, opus-review) merge в main + **задеплоен в прод** (asset `CRXk9JKF`, сверено dist=nginx:8080=снаружи HTTP 200, туннель восстановлен) + миграция `merchant_rules` применена к живой БД по «го». ⚠️ Банк НЕ прогнан на реальной выписке — проверить первый реальный импорт. real-time push отложен)

...
**Обновлено: 2026-08-11** (`main`/`origin/main` = `3ca355d`; MCP 6.7 внедрён, frontend с инструкцией для LLM развёрнут в production.)
...
- Новая миграция БД не нужна. Публичный HTTP endpoint намеренно не открыт: текущий безопасный production-режим — stdio к живой Supabase под пользовательским JWT; удалённая публикация требует OAuth/refresh-потока, HTTPS reverse proxy и allowlist.
...
- Удалённый MCP URL — только отдельным этапом с OAuth/refresh пользовательского JWT; локальный stdio production-доступ уже готов.

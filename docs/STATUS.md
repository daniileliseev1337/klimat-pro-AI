...
**Обновлено: 2026-07-04** (сессия: follow-up Фазы 3 `client_stage_changed` при смене стадии через форму проекта — в проде; brainstorm «умного импорта выписки Яндекс Банка v2» → spec одобрен, real-time push-канал проверен и отложен)

## Прод
- Живой: https://193-124-130-236.sslip.io — HTTP 200 снаружи (проверено exa) и на nginx :8080.
- Версия: **a0c8542** (follow-up Фазы 3 `client_stage_changed` через форму проекта + spec банка v2; asset
  `index-CG_qyP3W.js`, задеплоен 04.07, сверено dist=nginx:8080=снаружи, HTTP 200). GitHub origin/main = локальный main (a0c8542).
...
- 🟡 **Умный импорт выписки Яндекс Банка v2 (В РАБОТЕ — spec одобрен 04.07)**. Brainstorm проведён: real-time
  push-канал проверен и ОТЛОЖЕН (email/SMS у владельца не приходят, API физлицу-покупателю нет, push-перехват
  хрупок). Эта итерация — умный импорт: докрутка `parsePdfYandex` + классификация (оплата/перевод-себе/физлицу,
  обезличивание ПДн) + обучаемый категоризатор-цепочка (выученное→MCC→словарь-границы-слов→[LLM опц.]→ручной
  review) + `merchant_rules` в БД + дедуп. Вынос в `src/lib/bankParsers.js` + TDD на обезличенных сэмплах.
  Spec: `docs/superpowers/specs/2026-07-04-bank-import-v2-design.md`; **план готов**:
  `docs/superpowers/plans/2026-07-04-bank-import-v2.md` (10 задач TDD, коммит `08f3909`). Следующий шаг:
  **реализация по плану** (subagent-driven/executing-plans) — рекомендуется в свежем чате (эта была длинной).

...
**Обновлено: 2026-08-11** (`codex/mcp-site-access`: MCP-слой реализован и проходит локальные transport/security tests; `main`, живая БД и production пока не изменялись.)

## В работе — MCP-доступ LLM
...
- MCP 48/48, frontend 161/161, production build и `npm audit` (0 vulnerabilities) зелёные. Реальные дочерний stdio и локальный HTTP SDK-клиент проходят initialize/tools-list.
- Новая миграция БД не нужна. До фактического использования остаются owner-gates: merge в `main`; настройка конкретного MCP-клиента; production HTTP deploy — только если нужен удалённый URL.

## Готово — исправление персонализируемого UI

- Удалены структурные appearance-обёртки, которые создавали двойные золотые рамки и меняли размеры секций. Скин и эффект теперь декорируют существующую карточку без дополнительного DOM-контейнера.
- Индивидуальные настройки оставлены только для реальных контентных поверхностей: KPI/карточки Дашборда, карточки Проектов и Задач, финансовые карточки и операции. Ложные цели «управление», «фильтры» и «редактор» удалены.
- В каталог добавлены реальные зоны Admin: пользователи, статистика и журнал. Форма создания пользователя переведена на `StyledInput`/`StyledSelect` и адаптивную сетку.
- Режим настройки переоформлен в стиле сайта: компактное чёрно-золотое окно, группы панелей, визуальные образцы каждого скина/эффекта и рабочий live-preview.
- Закрыты замечания независимого review: Admin-цели скрыты для не-admin; `scan` проходит через preview; preview получает pointer для `tilt/spotlight`; Admin-form не переполняет промежуточные ширины; старый inline-tilt `Card/KpiCard` отключается на appearance-managed карточках и больше не перекрывает выбранный эффект.
- Visual QA: desktop PASS; mobile 390 px PASS без горизонтального overflow; `Блик` подтверждён вычисленным `animation-name: kp-ui-shimmer`, `6s` и видимым gradient-layer. После deploy выполнен smoke nginx-бандла: новые JS/CSS загружаются, console errors отсутствуют, горизонтального overflow на публичном экране входа нет. Полная авторизованная production-QA остаётся отдельной проверкой с пользовательской сессией.

## Готово — персонализируемый UI

- Добавлены 15 скинов и 15 эффектов с ограниченной compatibility matrix, отдельный режим настройки из профиля и 13 индивидуально настраиваемых реальных поверхностей внутри Дашборда, Проектов, Финансов, Задач и Admin.
...
- Тесты текущего `main`: 161/161 зелёные. Build зелёный (чанк 1.25 MB).

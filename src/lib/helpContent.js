...
export const HELP_SECTIONS = [
  ...
  { tab: "admin", key: "mcp-api", title: "Подключение LLM через MCP/API",
    desc: "Claude, Codex и другие MCP-клиенты могут читать данные сайта и выполнять разрешённые изменения от имени отдельного пользователя. Запись всегда требует предварительного просмотра и точного подтверждения.",
    how: "На компьютере с репозиторием выполните npm run mcp:install, затем npm run mcp:login и добавьте mcp/src/stdio.js как stdio-сервер. Полная инструкция — в разделе «Подключение LLM» файла README.md." },
